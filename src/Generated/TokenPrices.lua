-- Auto-generated token price data
-- Last updated: 2025-07-24

PeaversCurrencyData = PeaversCurrencyData or {}

-- WoW Token prices across regions
PeaversCurrencyData.TokenPrices = {
  lastUpdated = "2025-07-24",

  regions = {
    US = {
      goldPrice = 292823,  -- Gold cost of a token
      realPrice = 20,      -- USD cost of a token
      currency = "USD",
      goldValue = 0.0000683006, -- USD value of 1 gold
    },
    EU = {
      goldPrice = 392902,  -- Gold cost of a token
      realPrice = 20,      -- EUR cost of a token
      currency = "EUR",
      goldValue = 0.0000509033, -- EUR value of 1 gold
    },
    KR = {
      goldPrice = 176420,  -- Gold cost of a token
      realPrice = 22000,      -- KRW cost of a token
      currency = "KRW",
      goldValue = 0.1247024147, -- KRW value of 1 gold
    },
    TW = {
      goldPrice = 652991,  -- Gold cost of a token
      realPrice = 500,      -- TWD cost of a token
      currency = "TWD",
      goldValue = 0.0007657073, -- TWD value of 1 gold
    },
  }
}
